...
de_genes <- read.xlsx("./analysis/differential_expression/output/de_genes_mouse_liver_network.xlsx", rowNames = TRUE, colNames = TRUE)
mouse_liver <- read.xlsx("./data/mouse_liver_subset.xlsx")
metadata <- read.table("./data/mouse_liver_metadata.txt", header = TRUE, sep = ",")
...
write.table(microarray_final, file = "./analysis/banjo/input/mouse_liver_microarray_data.txt", sep = "\t", quote = FALSE, row.names = FALSE)

#check
rm(list=ls())
ma_data <- read.table("./analysis/banjo/input/mouse_liver_microarray_data.txt", sep = "\t", header = TRUE)
